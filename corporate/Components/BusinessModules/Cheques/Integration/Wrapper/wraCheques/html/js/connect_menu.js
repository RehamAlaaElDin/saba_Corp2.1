...
MenubarItem.prototype.init = function () {
  ...
  if (nextElement && nextElement.tagName === 'UL') {
    this.domNode.parentElement.classList.add('has-popout')
...
MenubarItem.prototype.handleKeydown = function (event) {
  ...
  switch (event.keyCode) {
    ...
    if( this.menu.orientation === 'vertical' ){
        ...
        flag = true;
      } else if (this.popupMenu && this.menu.orientation !== 'vertical') {
        this.popupMenu.open();
    ...
    case this.keyCode.LEFT:
      if( this.menu.orientation === 'vertical' && this.popupMenu ){
        this.popupMenu.open();
    ...
    default:
      if (isPrintableCharacter(chr)) {
        this.menu.setFocusByFirstCharacter(this, chr);
        flag = true;
      }
      break;
  }
...
var Menubar = function (domNode) {
  ...
  if ((domNode instanceof Element) === false) {
  ...
  this.orientation = 'horizontal' // See Menubar init method
};
...
Menubar.prototype.init = function () {
  ...
  //Check is menu orientation is vetical or horizontal
  if (this.domNode.parentElement.parentElement.matches('.menuvert')){
...
Menubar.prototype.setFocusByFirstCharacter = function (currentItem, chr) {
  var start, 
      index; 
  chr = chr.toLowerCase();
  ...
  index = this.getIndexFirstChars(start, chr);

  // If not found in remaining slots, check from beginning
  if (index === -1) {
    index = this.getIndexFirstChars(0, chr);
  }
...
Menubar.prototype.getIndexFirstChars = function (startIndex, chr) {
  for (var i = startIndex; i < this.firstChars.length; i++) {
    if (chr === this.firstChars[ i ]) {
      return i;
    }
  }
  return -1;
...
PopupMenu.prototype.setFocusByFirstCharacter = function (currentItem, chr) {
  var start, index, chr = chr.toLowerCase();
...
PopupMenu.prototype.getIndexFirstChars = function (startIndex, chr) {
  for (var i = startIndex; i < this.firstChars.length; i++) {
    if (chr === this.firstChars[ i ]) {
      return i;
    }
...
PopupMenu.prototype.open = function () {
  ...
  // Set CSS properties
  if ( !this.controller.isMenubarItem ||
    this.controller.menu.orientation == 'vertical'
    ) {
