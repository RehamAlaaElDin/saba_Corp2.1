...
function validateRegularExpression(c, expr, min, max, message, ns, inline, styles){
  ...
  if (hasChanged(ns, c))
  {
	  if (!this["beforeValidation"] || this["beforeValidation"](c, getMsg(ns, "ValidationMessage", "QUESTION_TEXT=" + getLabelText(ns, c)), null, null, min, max, message, null, null, ns, null, null, styles) ){
		if (!this["beforeValidateRegularExpression"] || beforeValidateRegularExpression(c, expr, min, max, message, ns, styles) ){
		  ...
		  if (expr != ''){
			...
			if (regExp && !regExp.test(c.value)){
		    if (message[0]!= null && message[0].length > 0) alertComp(c,message[0],ns, inline, styles);
		    else alertComp(c, getMsg(ns, "ValidationMessage", "QUESTION_TEXT=" + getLabelText(ns, c)), ns, inline, styles);
		    ok = false;
		   }
		  }
	  ...
	  {
		 ok = afterValidation(c, getMsg(ns, "ValidationMessage", "QUESTION_TEXT=" + getLabelText(ns, c)), null, null, min, max, message, null, null, ns, null, null, ok, styles);
	  }
		if (!ok)
		{
...
function getLabelText(ns, inputElem) {
    if (!inputElem || !inputElem.id) {
        return "";
    }
    var inputElemType = inputElem.getAttribute('type');
    if (inputElemType == 'checkbox' || inputElemType == 'radio') {
        //get element id from ifp
        var idWithoutCompPref = stripPrefix(ns, inputElem.id);
        var compID = getCompID(ns, idWithoutCompPref);
        if (compID == null) compID = "";
        //create span id (multiple checkbox and radio button has a span as question text
        var spanLabelId = ns + compID + "span_" + idWithoutCompPref;
        var checkboxInputRegexp = new RegExp("(_[0-9]+)$");
        var matched = spanLabelId.match(checkboxInputRegexp);
        //extract the number from the id (
        if  ( matched != null && matched.length > 0 )
        {
            var sufixCheckboxIndex = matched[0];
            spanLabelId = spanLabelId.substring(0, spanLabelId.length - sufixCheckboxIndex.length);
        }
        if (spanLabelId) {
            var spanElem = document.getElementById(spanLabelId);
            if (spanElem) {
                return decodeHtml(spanElem.innerHTML);
            }
        }
    }
    //try to find the ancestor with the id that contains "row" and get the label child
    var rowElem = getParentWithRowId(inputElem);
    var labelElem = rowElem.querySelector("label[for='" + inputElem.id + "']")
    if (labelElem) {
        return decodeHtml(labelElem.textContent);
    }
    //if no label found, get aria-label attribute
    var ariaLabelElem = inputElem.getAttribute('aria-label');
    if (ariaLabelElem)
    {
        return decodeHtml(ariaLabelElem);
    }
    return "";
}

function getParentWithRowId(elem) {
    for ( ; elem && elem !== document; elem = elem.parentNode ) {
        var idAttribute = elem.getAttribute('id');
        if ( idAttribute && ( idAttribute.indexOf('row_') >= 0 )) return elem;
    }
    return null;
}
...
function validFile(comp, validFileExts, min, max,  message, ns, inline, styles){
 var ok = false;
 var val = comp.value;
 if (!this["beforeValidation"] || this["beforeValidation"](comp, getMsg(ns, "InvalidUploadTypeMessage", "QUESTION_TEXT=" + getLabelText(ns, comp)), null, null, min, max, message, validFileExts, null, ns, null, null, styles) ){
  if (!this["beforeValidFile"] || beforeValidFile(comp, validFileExts, message, ns) ){
   ...
   if (!ok){
    ...
    if (validFileExts!=null && validFileExts.length>0 && !validateFileExtension(comp, validFileExts)){
     ...
     errorMessage += getMsg(ns, "InvalidUploadTypeMessage", "QUESTION_CONSTRAINT=" + validFileTypeStr, "QUESTION_TEXT=" + getLabelText(ns, comp));
     anyErrors=true;
    }
    if (max!=null && max.length>0 && !validateFileSize(comp,max)){
     if(anyErrors)errorMessage += "&nbsp;&nbsp;";
     errorMessage += getMsg(ns, "InvalidMaxFileSizeMessage", "QUESTION_CONSTRAINT=" + max, "QUESTION_TEXT=" + getLabelText(ns, comp));
     anyErrors=true;
    }
 ...
 if (this["afterValidation"]) 
 {
     ok = afterValidation(comp, getMsg(ns, "InvalidUploadTypeMessage", "QUESTION_TEXT=" + getLabelText(ns, comp)), null, null, min, max, message, validFileExts, null, ns, null, null, ok, styles);
 }
 // make sure the form can support file upload
 if (ok)
 {
...
function validAlphaNumeric(comp, min, max, message, ns, inline, styles){
 return checkWithHook("ValidAlphaNumeric", comp, "AN", getMsg(ns, "InvalidAlphaNumericMessage", "QUESTION_TEXT=" + getLabelText(ns, comp)), null, null, min, max, message, null, null, ns, inline, null, null, styles);
}

function validAlpha(comp, min, max, message, ns, inline, styles){
 return checkWithHook("ValidAlpha", comp, "A", getMsg(ns, "InvalidAlphaMessage", "QUESTION_TEXT=" + getLabelText(ns, comp)), null, null, min, max, message, null, null, ns, inline, null, null, styles);
}
...
function validNum(comp, min, max, numberSymbols, message, ns, inline, styles, name, numType, invalidMessage, regEx) {
 var isOk = checkWithHook(name, comp, numType, getMsg(ns, invalidMessage, "QUESTION_TEXT=" + getLabelText(ns, comp)), numberSymbols, null, min, max, message, null, null, ns, inline, null, null, styles);
 var num = comp.value;
 if (isOk && numberSymbols && numberSymbols.length > 0)
 {
   ...
   {
     if(num.charAt(0) =='-') num = '-'+num.substr(1).replace(regEx, "");
     else num = num.replace(regEx, "");

     var newnum = addNumberFormating(num, numberSymbols);
     if (newnum != num) 
      {
           comp.value = newnum;
     }
...
function validNumeric(comp, min, max, numberSymbols, message, ns, inline, styles){
	return validNum(comp, min, max, numberSymbols, message, ns, inline, styles, "ValidNumeric", "N", "InvalidNumMessage", /^0+(?!$)/);
}
function validDouble(comp, min, max, numberSymbols, message, ns, inline, styles){
	return validNum(comp, min, max, numberSymbols, message, ns, inline, styles, "ValidDouble", "D", "InvalidDecimalMessage", /^0+(?!\.|$)/);
}
...
function validDateTime(dateObj, min, max, message, format, separator, ns, inline, styles){
 ...
 return checkWithHook("ValidDateTime", dateObj, "VDT", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + unescape(format[0] + separator[2] + format[1]), "QUESTION_TEXT=" + getLabelText(ns, dateObj)), null, null, min, max, message, format, separator, ns, inline, null, null, styles);
}
...
function validDateWeekTime(dateObj, min, max, message, format, separator, ns, inline, styles){
 ...
 if (separator == null) separator = new Array("/", ":", " ");
 return checkWithHook("ValidDateWeekTime", dateObj, "VDWT", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + unescape(format[0] + separator[2] + format[1]), "QUESTION_TEXT=" + getLabelText(ns, dateObj)), null, null, min, max, message, format, separator, ns, inline, null, null, styles);
}

function validDateWeek(dateObj, min, max, message, format, separator, ns, inline, styles){
 if (format == null) format = "ww/yyyy";
 if (separator == null) separator = "/";
 return checkWithHook("ValidDateWeek", dateObj, "VDW", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + unescape(format), "QUESTION_TEXT=" + getLabelText(ns, dateObj)), null, null, min, max, message, format, separator, ns, inline, null, null, styles);
}

function validDate(dateObj, min, max, message, format, separator, ns, inline, styles){
 if (format == null) format = "dd/mm/yyyy";
 if (separator == null) separator = "/";
 return checkWithHook("ValidDate", dateObj, "VD", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + unescape(format), "QUESTION_TEXT=" + getLabelText(ns, dateObj)), null, null, min, max, message, format, separator, ns, inline, null, null, styles);
}

function validTime(dateObj, min, max, message, format, separator, ns, inline, styles){
 return checkWithHook("ValidTime", dateObj, "VT", getMsg(ns, "InvalidTimeMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, dateObj)), null, null, min, max, message, format, separator, ns, inline, null, null, styles); 
}
...
function validDayDatePart(dayObj, monthObjName, yearObjName, min, max, clientMin, clientMax, message, format, separator, ns, inline, styles){
 var res = checkWithHook("ValidDayDatePart", dayObj, "VDP", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, dayObj)), monthObjName, yearObjName, min, max, message, format, separator, ns, inline, clientMin, clientMax, styles);
...
function validMonthDatePart(monthObj, dayObjName, yearObjName, min, max, clientMin, clientMax, message, format, separator, ns, inline, styles){
 var res = checkWithHook("ValidMonthDatePart", monthObj, "VMP", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, monthObj)), dayObjName, yearObjName, min, max, message, format, separator, ns, inline, clientMin, clientMax, styles);
...
function validYearDatePart(yearObj, monthObjName, dayObjName, min, max, clientMin, clientMax, message, format, separator, ns, inline, styles){
 var res = checkWithHook("ValidYearDatePart", yearObj, "VYP", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, yearObj)), dayObjName, monthObjName, min, max, message, format, separator, ns, inline, clientMin, clientMax, styles);
...
function validHourTimePart(hourObj, minObjName, secObjName, min, max, clientMin, clientMax, message, format, separator, ns, inline, styles){
 var res = checkWithHook("ValidHourTimePart", hourObj, "VHT", getMsg(ns, "InvalidTimeMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, hourObj)), minObjName, secObjName, min, max, message, format, separator, ns, inline, null, null, styles);
...
function validMinuteTimePart(minObj, hourObjName, secObjName, min, max, clientMin, clientMax, message, format, separator, ns, inline, styles){
 var res = checkWithHook("ValidMinuteTimePart", minObj, "VMT", getMsg(ns, "InvalidTimeMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, minObj)), hourObjName, secObjName, min, max, message, format, separator, ns, inline, null, null, styles);
...
function validSecondTimePart(secondObj, hourObjName, minObjName, min, max, clientMin, clientMax, message, format, separator, ns, inline, styles){
 var res = checkWithHook("ValidSecondTimePart", secondObj, "VST", getMsg(ns, "InvalidTimeMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, secondObj)), hourObjName, minObjName, min, max, message, format, separator, ns, inline, null, null, styles);
...
function validWeekDatePart(weekObj, yearObjName, min, max, clientMin, clientMax, message, format, separator, ns, inline, styles){
 var res = checkWithHook("ValidWeekDatePart", weekObj, "VWP", getMsg(ns, "InvalidDateMessage", "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, weekObj)), "", yearObjName, min, max, message, format, separator, ns, inline, clientMin, clientMax, styles);
 return res;
}
...
function checkA(c, t, m, d1, d2, min, max, message, format, separator, ns, inline, clientMin, clientMax, styles) {
 ...
 if (!ok){
  ...
  var errorMsg = ( message[errorMessageIndex] && message[errorMessageIndex].length > 0) ? getMsg("@GOT_MSG@", message[errorMessageIndex], "QUESTION_CONSTRAINT=" + format, "QUESTION_TEXT=" + getLabelText(ns, c)) : m;
  if(!errorMsg){
    errorMsg = getMsg(ns, "ValidationMessage", "QUESTION_TEXT=" + getLabelText(ns, c));
  }
...
function isLegalExpresion(c, styles) {
	...
	if (tmp != null && tmp.length != 0) {
		...
		if (regExp.test(c.value)){
			alertComp(c, getMsg("", "ValidationMessage", "QUESTION_TEXT=" + getLabelText(ns, c)), "", true, styles);
			return false;
		}
	}
	return true;
...
function checkMax(comp, val, max, text, message, ns, inline, numberSymbols, styles){
 ...
 if (val1 > val2){
  if (message[2].length > 0) alertComp(comp, message[2], ns, inline, styles);
   ...
   alertComp(comp,getMsg(ns, "InvalidMax" + text + "Message", "QUESTION_CONSTRAINT=" + val2, "QUESTION_TEXT=" + getLabelText(ns, comp)),ns, inline, styles);
   }
  return false;
 }
 return true;
...
function checkMin(comp, val, min, text, message, ns, inline, numberSymbols, styles){
 ...
 if (val1 < val2){
  if (message[1].length > 0) alertComp(comp,message[1],ns, inline, styles);
   ...
   if (numberSymbols && numberSymbols.length > 0) val2 = addNumberFormating(val2, numberSymbols);
   alertComp(comp,getMsg(ns, "InvalidMin" + text + "Message", "QUESTION_CONSTRAINT=" + val2, "QUESTION_TEXT=" + getLabelText(ns, comp)),ns, inline, styles);
  }
  return false;
 }
 return true;
...
function validDateParts(focusObj, dayObj, monthObj, yearObj, min, max, clientMin, clientMax, format, separator, ns, inline, message, styles){
 ...
 if (intDay > daysInMonth[intMonth]){
  return alertComp(dayObj, ( message[3] && message[3].length > 0 ) ? message[3] : getMsg(ns, "InvalidDaysInMonthMessage", "QUESTION_TEXT=" + getLabelText(ns, dayObj)), ns, inline, styles);
 }
 else if ((intMonth == 2) && (intDay > daysInFebruary(intYear))){
  return alertComp(yearObj, ( message[4] && message[4].length > 0 ) ? message[4] : getMsg(ns, "InvalidLeapYearMessage", "QUESTION_TEXT=" + getLabelText(ns, yearObj)), ns, inline, styles);
 }
...
function checkMaxDate(comp, val, max,  format, sep, ns, inline, clientMax, message, styles){
 ...
 if (!retVal){
  return alertComp(comp, ( message[2] && message[2].length > 0 ) ? message[2] : getMsg(ns, "InvalidMaxDateMessage", "QUESTION_CONSTRAINT=" + ((clientMax == null) ? max : clientMax), "QUESTION_TEXT=" + getLabelText(ns, comp)), ns, inline, styles);
 }
 return true;
}
...
function checkMinDate(comp, val, min, format, sep, ns, inline, clientMin, message, styles){
 ...
 if (!retVal){
  return alertComp(comp, ( message[1] && message[1].length > 0 ) ? message[1] : getMsg(ns, "InvalidMinDateMessage", "QUESTION_CONSTRAINT=" + ((clientMin == null ) ? min : clientMin), "QUESTION_TEXT=" + getLabelText(ns, comp)), ns, inline, styles);
 }
 return true;
}
...
function alertMandMessage(mandComp, ns, inlineErrors){
  ...
  if (mandMessage == "Mandatory") {
	mandMessage = getMsg(ns, "MandMessage", "QUESTION_CONSTRAINT=" + getVariable(ns, "MANDCHAR"), "QUESTION_TEXT=" + getLabelText(ns, mandComp));
  }
  else {
    mandMessage = substituteVariable(mandMessage, "QUESTION_CONSTRAINT=" + getVariable(ns, "MANDCHAR"));
    mandMessage = substituteVariable(mandMessage, "QUESTION_TEXT=" + getLabelText(ns, mandComp));
  }
...
function showErrorMessage(ns, comp, p_msg, styles) {
	...
	if (errorNode){
		...
		//search if there is a visible warning or info message after the error
		var warningMessageId = getWarningMessageId(ns, comp);
		var infoMessageId = getInfoMessageId(ns, comp);
		var lastMessageElement = errorNode;
		if (lastMessageElement.nextSibling != null)
		{
			while (lastMessageElement.nextSibling.id == warningMessageId || lastMessageElement.nextSibling.id == infoMessageId) {
				lastMessageElement = lastMessageElement.nextSibling;
				if (lastMessageElement.style.display != "none" || lastMessageElement.nextSibling == null) {
					break;
				}
			}
		}
		...
		errorNode.innerHTML = '';
		var newMessage = p_msg;
		if(lastMessageElement != errorNode && lastMessageElement.style.display != "none" && getVariable(ns, "ShowOneMessage") != "Y") {
			// if the error is not the last visible message just add the separator - not the case when only one message (the error) is shown
			newMessage += getVariable(ns, "MessagesSeparator");
		}
		errorNode.innerHTML = newMessage;
...
function mandCheckRow(p_rowId, ns, p_butAction, inlineErrors, validateAllFields)
{
 ...
 if (mandComp != null){
  ok = false;
  alertMandMessage(mandComp, ns, inlineErrors);
  setFocus(mandComp);
 }
            ...
            for (var i=0; i < DATE_TIME_PARTS.length; i++)
            {
                ...
                if  (otherElemKey != elemKey)
                {
                    ...
                    if  (otherElem)
                    {
                        ...
                        if  (trim(val) == "") 
                        {
                            return alertComp(otherElem, getMsg(ns, "ValidationMessage", "QUESTION_TEXT=" + getLabelText(ns, otherElem)), ns, inline);
                        }
                    }
                }
            }
...
function trimValue(elem, ns)
{
	elem.value = trim(getElementValue(elem, ns));
}
