...
$%SET CURRENT_LANGUAGE_ISO_CODE$$%IF LANGUAGE_MAP_CODE != ''$$$LANGUAGE_MAP_CODE$$%ELSE$en$%ENDIF$$%ENDSET$
  <html dir="$$PRESENTATIONTYPE.LAYOUTDIRECTION$" xmlns="http://www.w3.org/1999/xhtml" xml:lang="$$!CURRENT_LANGUAGE_ISO_CODE$" lang="$$!CURRENT_LANGUAGE_ISO_CODE$" >
    <head>
      ...
      $%COMMENT$ enable antiFlickeringMode before $$HEADCONTENT$ if you experience screen rendering or network related issues that cause screen flickering on slow page transitions. See connect.js for more details for what it does.
      This should be a last resort; ideally the UXP solution (screen content, styles, custom scripts) and hosting hardware should be optimized so that this issue does not occur in the first place.
      <script type="text/javascript">var antiFlickeringMode = true;</script>
      $%ENDCOMMENT$
                $$HEADCONTENT$
...
$%endif$
	$%if IN_POPUP != "Y"$ $%COMMENT$ don't show the branding in a popup $%ENDCOMMENT$
	    <div class="Banner" role="banner"></div>
	$%endif$
      $%COMMENT$ To center the contents of your form, set style="text-align:center;" on the form tag (works in IE), 
          and style="margin-left:auto; margin-right:auto;" on the following table (works in Mozilla) $%ENDCOMMENT$
          ...
          $%endif$
          $%COMMENT$
		  To be accessible, if the structure of a page doesn't fit to the standard way, then the role="main" attribute can be removed, moved to another section inside the solution or another template can be used.
          $%ENDCOMMENT$
		  <div role="main"> 
             $$FORMCONTENT$
		  </div>
