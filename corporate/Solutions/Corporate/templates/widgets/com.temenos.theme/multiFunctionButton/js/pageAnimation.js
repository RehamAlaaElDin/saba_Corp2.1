...
$(function() {
    ...
    function beforeAnimation(controllerName, ns, context, btn, disableInput, buttonId) {
        ...
        isOverlayFromTopShift    = buttonClass.indexOf("ecAnimOverlayFromTopShift") > -1 ? true : false;
        isOverlayFromRightShift  = buttonClass.indexOf("ecAnimOverlayFromRightShift") > -1 ? true : false;
        isOverlayFromBottomShift = buttonClass.indexOf("ecAnimOverlayFromBottomShift") > -1 ? true : false;
        isOverlayFromLeftShift   = buttonClass.indexOf("ecAnimOverlayFromLeftShift") > -1 ? true : false;
        ...
        isRemoveToTopUnShift    = buttonClass.indexOf("ecAnimRemoveToTopUnShift") > -1 ? true : false;
        isRemoveToRightUnShift  = buttonClass.indexOf("ecAnimRemoveToRightUnShift") > -1 ? true : false;
        isRemoveToBottomUnShift = buttonClass.indexOf("ecAnimRemoveToBottomUnShift") > -1 ? true : false;
        isRemoveToLeftUnShift   = buttonClass.indexOf("ecAnimRemoveToLeftUnShift") > -1 ? true : false;
        ...
        if (isSRUG || isSRRG  || isSRLG || isSRDG  || isScrollUp ||  isScrollDown || isLeft  || isRight || isOffLeftFadeIn || isFadeOutInLeft || isFadeOutFadeIn || isFlipForwards || isFlipBackwards|| isFlipVertTop || isFlipVertBottom ||
             isOverlayFromTop || isOverlayFromLeft || isOverlayFromBottom || isOverlayFromRight || isOverlayFromTopShift   || isOverlayFromRightShift || isOverlayFromBottomShift  || isOverlayFromLeftShift || 
             isRemoveToTop    || isRemoveToLeft    || isRemoveToBottom    || isRemoveToRight    || isRemoveToTopUnShift    || isRemoveToRightUnShift  || isRemoveToBottomUnShift   || isRemoveToLeftUnShift  ||
             isTurnPageNext   || isTurnPagePrev) {

            //unsupported IE animations
            if ($("html").is(".ie") && (buttonClass.indexOf("Flip") > 0 || buttonClass.indexOf("Cube") > 0) ){
                inAnim = false;
                return true;
            }
            ...
            animSpeed =  (animParams.length > 1) ? animParams[1] : defaultAnimSpeed;
            animType  =  (animParams.length > 2) ? animParams[2] : defaultAnimType;

            //hide anything for animation...
            if ($("body").get(0).scrollHeight <= $(window).height() )
                $("html,body").css("overflow", "hidden");
            $(".hideForAnimation").fadeTo(100, 0);
        }
        else
            inAnim = false;
    ...
    function setupCurrentContainer() {
        ...
        var tmp =  $($("#" + idToUpdate)[0].cloneNode(false));
        tmp.css({"position":"absolute", })
        $("#" + idToUpdate).attr("id", oldContentId).after(tmp)
    }
    function setupNewContainer(text, style) {
        ...
        text = text.slice(0, styleIndex + 7) + "position: absolute; width: 100%; top: 0px; left: 0px;" + style  + text.slice(styleIndex + 7);
        return text;
    ...
    function afterAnimationResponseReceived(service, element, innerOrOuter, text, actionFlag, actionData, ajaxCaller, dElement, ns) {
        ...
        if (element == idToUpdate && innerOrOuter == "OUTER") {
            ...
            if (isLeft || isRight || isScrollDown || isScrollUp || isOverlayFromTop || isOverlayFromLeft || isOverlayFromRight || isOverlayFromBottom ||
                isOverlayFromTopShift || isOverlayFromRightShift || isOverlayFromBottomShift || isOverlayFromLeftShift ) {
                setupCurrentContainer();
                text = setupNewContainer(text, "z-index: 20;");

                //work out where to put new anim section
                var offsetX = 0, offsetY = 0;
                if (isLeft || isRight  || isOverlayFromLeft || isOverlayFromRight || isOverlayFromRightShift || isOverlayFromLeftShift) {
                    offsetX =  (isLeft||isOverlayFromLeft|| isOverlayFromLeftShift ? "-": "") + $(window).width()  + "px";
                    offsetY = window.pageYOffset + "px";
                }
                if (isScrollDown || isScrollUp || isOverlayFromTop || isOverlayFromBottom || isOverlayFromTopShift || isOverlayFromBottomShift)
                        offsetY = (isScrollDown||isOverlayFromTop || isOverlayFromTopShift ? "-": "") + $(window).height() + "px";

                
                text = setupAnimSection(text, {"opacity": "1", "overflow":"hidden"}, "translate3d(" + offsetX + ","+offsetY+ ",0)");
            }
                ...
                isRemoveToTopUnShift || isRemoveToRightUnShift || isRemoveToBottomUnShift || isRemoveToLeftUnShift ) {
                setupCurrentContainer();
                text = setupNewContainer(text);
                ...
                console.log("Animation unrecognised: ");
                return true;
            }
    ...
    function doAnimation(service, element, innerOrOuter, text, actionFlag, actionData, ajaxCaller, dElement, ns) {
        if (element == idToUpdate && innerOrOuter == "OUTER") {  
            ...
            $(".hideForAnimation").css("opacity", 0);            
            ...
            if (isSRDG || isSRLG || isSRRG || isSRUG) {
                ...
                if (isOverlayFromRightShift || isOverlayFromLeftShift)
                    offsetX = (isOverlayFromRightShift ? "-" : "") + 50;
                if (isOverlayFromTopShift || isOverlayFromBottomShift)
                    offsetY = (isOverlayFromBottomShift ? "-" : "") + 50;
                ...
                if (isOverlayFromRightShift  || isOverlayFromLeftShift || isOverlayFromTopShift || isOverlayFromBottomShift) {
                    $("#" + oldContentId + " #" + idToAnimate).transition({
                        x: offsetX,
                        y: offsetY
                    }, animSpeed, animType); 
                ...
                $("#" + idToUpdate  + " #" + idToAnimate).transition({
                    x: 0,
                    y: (isLeft || isRight  || isOverlayFromLeft || isOverlayFromRight || isOverlayFromRightShift || isOverlayFromLeftShift) 
                        ? window.pageYOffset 
                        : 0
                }, animSpeed, animType, function() {
                     _tidyUp()
                ...
                var xOffset = 0, yOffset = 0,
                    xShiftOffset = 0, yShiftOffset = 0;

                if (isRemoveToRight || isRemoveToLeft) {
                    xOffset = (isRemoveToLeft ? "-" : "") +  $(window).width() + "px"
                } 
                else if (isRemoveToTop || isRemoveToBottom) {
                    yOffset = (isRemoveToTop ? "-" : "") +  $(window).height() + "px"
                }

                if (isRemoveToRightUnShift || isRemoveToLeftUnShift) {
                    xShiftOffset = (isRemoveToRightUnShift ? "-" : "") + 50;
                }
                else if (isRemoveToTopUnShift || isRemoveToBottomUnShift) {
                    yShiftOffset = (isRemoveToBottomUnShift ? "-" : "") + 50;
                }

                if (isRemoveToTopUnShift || isRemoveToRightUnShift || isRemoveToBottomUnShift || isRemoveToLeftUnShift) {
                    $("#" + idToUpdate  + " #" + idToAnimate).transition({x: xShiftOffset, y: yShiftOffset}, 0)
                    $("#" + idToUpdate  + " #" + idToAnimate).transition({
                        x: 0,
                        y: 0
                    },animSpeed, animType)
    ...
    function _tidyUp() {
        if ("TT_Module" in window) 
            TT_Module.hideTooltips();    
        $("#DELETE_ME, .DELETE_ME").remove();
        $("#" + idToUpdate  + ",#" + idToAnimate).css({"position": "","width": "","height": "", "overflow": "", "z-index": "", "transform": ""})
        $(".hideForAnimation").fadeTo(400, 1)
        $("html,body").css("overflow", "");

        inAnim = false;        
        UtilModule.fixFooter();
        UtilModule.scrollIt(0, 0, 'easeOutQuad')
    }
